...
import { playerNameAtom, playerImageAtom, overallAtom, rarityAtom, nationAtom, leagueAtom, positionsAtom, alternatepositionAtom, playstylesAtom, weakAtom, skillmovesAtom, footAtom, defensiveAtom, atackingAtom, pacAtom, shoAtom, phyAtom, pasAtom, defAtom, driAtom, searchQ, searchResult, isSearchResultShowing, selectedItemsAtom } from './Atom/GlobalStates';
import { useAtom } from 'jotai';
import { toast } from 'react-toastify';

const CardPageSlug = ({ slug }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch(`${BaseURL}/players/${slug}`);
    ...
    console.log('AMAR', selectedItems);
                ...
                <div id='card-picture' ref={cardPictureRef} className="relative w-[280px] h-[391.453px] mb-[20px] bg-transparent">
                    ...
                    >
                        <div
                            ...
                            style={{
                                backgroundImage: `url(${selectedItems.image})`,
                            }}
                        ></div>
                    </div>
                    ...
                    <div className="absolute text-white font-500 text-xs z-2 bg-transparent top-[275px] h-[fit-content] w-[full] left-10 right-10 text-center flex gap-3 items-center justify-center">
                        <div className="bg-transparent text-white">
                            PAC<br />{selectedItems.pace ? selectedItems.pace : 0}
                        </div>
                        <div className="bg-transparent text-white">
                            SHO<br />{selectedItems.shot ? selectedItems.shot : 0}
                        </div>
                        <div className="bg-transparent text-white">
                            PAS<br />{selectedItems.pass ? selectedItems.pass : 0}
                        </div>
                        <div className="bg-transparent text-white">
                            DRI<br />{selectedItems.dribbling ? selectedItems.dribbling : 0}
                        </div>
                        <div className="bg-transparent text-white">
                            DEF<br />{selectedItems.defense ? selectedItems.defense : 0}
                        </div>
                        <div className="bg-transparent text-white">
                            PHY<br />{selectedItems.physical ? selectedItems.physical : 0}
                        </div>
                    </div>


                    {(selectedItems.playStyle).slice(0, 4).map((style, index) => (
                        <div
                            key={index}
                        ...
                        >
                            <div className="w-full h-full flex mt-[7px] justify-center bg-transparent">
                                <Image
                                    src={selectedItems.playStyle[index]} // Assuming each style object has an imagePath property
